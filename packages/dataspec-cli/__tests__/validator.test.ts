...
describe('Validation Engine', () => {
  ...
  describe('Resource name uniqueness', () => {
    ...
    it('should detect multiple resource types each having duplicates', () => {
      ...
      const result = validateWorkspace(workspace);
      expect(result.passed).toBe(false);
      expect(result.errors.some(e => e.code === 'DUPLICATE_SOURCE_NAME' && e.message.includes("'alpha'"))).toBe(true);
      expect(result.errors.some(e => e.code === 'DUPLICATE_DATASET_NAME' && e.message.includes("'beta'"))).toBe(true);
    });
