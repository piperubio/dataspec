...
// Compiled binary path - bun build --compile creates a native executable
...
`,
  ...
  await writeFile(
    join(dataspecPath, 'flows', 'test_flow.yaml'),
    `name: test_flow
steps:
  - type: extract
    source: test_db
...
describe('CLI Integration', () => {
  ...
  describe('dataspec validate', () => {
    ...
    it('should show validation errors in invalid workspace', async () => {
      ...
      await writeFile(
        join(dataspecPath, 'flows', 'bad.yaml'),
        'name: bad_flow\nsteps:\n  - type: extract\n    source: nonexistent\n    entity: test\n    output: out',
      );
  ...
  describe('dataspec list', () => {
    ...
    it('should list datasets', async () => {
      const result = await $`${CLI_PATH} list datasets --path ${testWorkspace}`;
      const output = result.stdout.toString();
  ...
  describe('dataspec show', () => {
    ...
    it('should return error for non-existent resource', async () => {
      const result =
        await $`${CLI_PATH} show dataset nonexistent --path ${testWorkspace}`.nothrow();
      expect(result.exitCode).toBe(2);
    });

    it('should show JSON format', async () => {
      const result =
        await $`${CLI_PATH} show dataset users_raw --path ${testWorkspace} --format json`;
      const json = JSON.parse(result.stdout.toString());
  ...
  describe('dataspec init', () => {
    ...
    it('should succeed in non-empty directory if dataspec folder does not exist', async () => {
      ...
      try {
        ...
        const result = await $`${CLI_PATH} init --path ${tempDir}`;
        expect(result.exitCode).toBe(0);
  ...
  describe('dataspec validate - workspace structure', () => {
    it('should fail when workspace has no dataspec folder', async () => {
      ...
      try {
        ...
        const result = await $`${CLI_PATH} validate --path ${noDataspecDir}`.nothrow();
        expect(result.exitCode).toBe(2);
    ...
    it('should pass when workspace has proper dataspec folder structure', async () => {
      ...
      try {
        ...
        const result = await $`${CLI_PATH} validate --path ${properDir}`;
        expect(result.exitCode).toBe(0);
      } finally {
...
steps:
  - type: extract
    ...
    it('should exit with code 2 for CLI errors', async () => {
      const result = await $`${CLI_PATH} validate --path /nonexistent/path`.nothrow();
      expect(result.exitCode).toBe(2);
    });

    it('should output parseable JSON for programmatic use', async () => {
      const result = await $`${CLI_PATH} validate --path ${testWorkspace} --format json`;
      const json = JSON.parse(result.stdout.toString());
...
steps:
  - type: extract
    ...
    output: out`,
      ...
      const result = await $`${CLI_PATH} validate --path ${invalidDir} --format text`.nothrow();
      const output = result.stdout.toString();
    ...
    it('should work with npx for CI without local install', async () => {
      ...
      const result = await $`${CLI_PATH} --version`;
      expect(result.exitCode).toBe(0);
    });
  });
});
