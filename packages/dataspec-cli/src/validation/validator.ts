...
export class Validator {
   ...
   */
  private detectTypeNarrowing(field: { name: string; type: string; constraints?: Record<string, unknown> }): string[] {
    const issues: string[] = [];
   ...
   */
  private detectConstraintTightening(field: { name: string; type: string; constraints?: Record<string, unknown> }): string[] {
    const issues: string[] = [];

    if (!field.constraints) return issues;
    ...
    if (field.constraints.enum) {
      issues.push(`enum constraint limits to ${(field.constraints.enum as unknown[]).length} allowed values`);
    }
  ...
  private getAffectedResources(contractName: string): Array<{ type: string; name: string }> {
    ...
    const affectedDatasets = this.workspace.datasets.filter(d => d.contract?.name === contractName);
