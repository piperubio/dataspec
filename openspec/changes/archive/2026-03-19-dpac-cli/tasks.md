...
- [x] 1.5 Setup test framework with test utilities
...
- [x] 2.5 Add error handling for malformed YAML files with line numbers
...
- [x] 3.6 Add utilities to traverse graph upstream/downstream from any resource

## 4. Validation Engine - Phase 1: Graph Integrity

- [x] 4.1 Implement cycle detection algorithm for flow dependencies
- [x] 4.2 Implement orphaned resource detection (datasets not produced or consumed)
- [x] 4.3 Implement incomplete pipeline detection (flow steps not forming complete pipeline)
...
- [x] 5.4 Implement unresolved flow reference validation in dataset produced_by declarations
...
- [x] 8.3 Implement type narrowing breaking change detection
- [x] 8.4 Implement constraint tightening breaking change detection
- [x] 8.5 Add multi-hop breaking change detection through the dependency chain
- [x] 8.6 Report all affected resources in breaking change errors
...
- [x] 9.5 Format output as `<file>:<line>:<severity>: <message>`
- [x] 9.6 Group output by severity (errors first, then warnings)
...
- [x] 13.5 Add global `--verbose` flag for debug output
...
- [x] 14.8 Add CI/CD integration tests
...
- [x] 15.4 Add JSDoc comments to all public APIs
