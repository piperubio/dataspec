...
**Rationale**: Different workloads need different engines. Spark for big data, DuckDB for analytics, dbt for SQL transforms, Python for custom logic. The DSL declares *which engine* a transform step uses without specifying *how* it runs — this is metadata for the implementing tool or validation engine, not execution configuration.
