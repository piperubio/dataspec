...
DataSpec core provides a validation engine that checks YAML DSL files for correctness, including graph integrity, cross-resource references, and breaking change detection via workspace dependency analysis. The CLI exposes this via `dataspec validate`, but this requires manual execution and doesn't integrate with the editor experience.
...
- Provide real-time diagnostics that match `dataspec validate` output
...
- Simple to spawn from CLI (`dataspec lsp`)
...
1. **Configuration:** Should the LSP server read `.dataspecrc` for custom validation rules? (Likely yes — consistency with CLI)
