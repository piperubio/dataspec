...
The system SHALL provide a CLI command `dpac inspect` that displays detailed information about platform resources.

#### Scenario: Inspect platform overview
- **WHEN** a user runs `dpac inspect`
...
- **WHEN** a user runs `dpac inspect dataset users_raw`
...
- **WHEN** a user runs `dpac inspect flow users_pipeline`
...
- **WHEN** a user runs `dpac inspect lineage users_analytics`
