...
- Provide `dpac inspect` CLI for interactive model exploration
...
### `dpac inspect` as CLI Command
**Decision**: Implement model inspection as a CLI subcommand with resource-type subcommands.

**Syntax**: `dpac inspect [resource-type] [resource-name]`

**Modes**:
- `dpac inspect` — platform overview (resource counts, storage backends)
- `dpac inspect dataset <name>` — dataset details + lineage
- `dpac inspect flow <name>` — flow steps and dependencies
- `dpac inspect lineage <dataset>` — upstream/downstream lineage tree

**Rationale**:
- Consistent with existing `dpac validate` and `dpac init` commands
...
| `dpac inspect` output too verbose | Low | Default to summary view; add `--verbose` flag for full details |
...
**Adoption path**:
1. Upgrade to DPaC version with docs support
2. Run `dpac docs generate` in existing workspace
3. Commit generated `docs/` directory
4. (Optional) Add CI check: `dpac docs generate --check` fails if docs out of sync

## Open Questions

1. Should `dpac inspect lineage` support depth limiting (e.g., `--depth 3`)?
